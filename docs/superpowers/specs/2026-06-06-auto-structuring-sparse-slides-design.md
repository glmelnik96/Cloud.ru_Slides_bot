...
**Implication for Phase 2 — pivot the metric.** If genuine underfill exists, it manifests as body slots filled with *trivially little text* (1–2 words), not as empty slots. The remedy detector should measure **content volume** (e.g. total body chars across slots below a floor, or per-slot chars tiny) rather than slot occupancy. The `content_chars` list is already in the `sparse_candidates` payload, but it only emits *when a slide is flagged* — and nothing flagged here, so no volume distribution was actually captured. Phase 2's first move should therefore be to emit volume telemetry for **every eligible** slide (the same widening the throwaway `sparse_eligible` probe did for occupancy), gather the char distribution across a larger corpus, then set the floor from data.
